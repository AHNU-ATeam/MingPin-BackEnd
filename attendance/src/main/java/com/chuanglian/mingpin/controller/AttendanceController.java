...
import com.chuanglian.mingpin.entity.vo.RequestDate;
...
//    LocalDate endDate = params.containsKey("endDate") ? LocalDate.parse((String) params.get("endDate")) : null;
    ...
    @PostMapping("/is_attended/stu")
    Result<List<Integer>> getStudentsIsAttended(@RequestBody RequestDate requestDate) {
        if (requestDate.getDate() == null) {
            requestDate.setDate(LocalDate.now());
        }
        return Result.success(stuAttendService.getStudentsIsAttended(requestDate.getDate()));
    }

    @PostMapping("/is_attended/emp")
    Result<List<Integer>> getEmployeeIsAttended(@RequestBody RequestDate requestDate) {
        if (requestDate.getDate() == null) {
            requestDate.setDate(LocalDate.now());
        }
