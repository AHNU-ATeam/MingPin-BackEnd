...
public interface CampMapper extends BaseMapper<Campus> {
    ...
    // 新建校区
//    @Insert("insert into campusManagement.campus (logo, name, region, address, principal_id, renewal_start,renewal_end,info,population, created_at,updated_at,campus_pics_urls,teacher_pics_urls) values " +
//            "(#{logo},#{name},#{region},#{address},#{principalId},#{renewalStart},#{renewalEnd},#{info},#{population},#{createdAt},#{updatedAt},#{campusPicsUrls},#{teacherPicsUrls})")
